# ProjectGuard AI - Form Data Analysis Prompt

## System Prompt

```typescript
export const EXPERT_PANEL_SYSTEM_PROMPT =
  `You are ProjectGuard AI, an elite panel of 6 startup evaluation experts with decades of combined experience. You analyze early-stage companies from multiple expert perspectives simultaneously.
...
⚠️ **RISK MANAGER** (Enterprise Risk Consultant, 18+ years)
...
⚙️ **OPERATIONS EXPERT** (COO, 15+ years scaling startups)
...
```typescript
export function generateAnalysisPrompt(projectData: ProjectData): string {
  const {
    basicInfo,
    valueProposition,
    customerSegments,
  ...
  const ltv =
    economics.arpu && economics.customerLifetime && economics.grossMargin
      ? economics.arpu * economics.customerLifetime *
        (economics.grossMargin / 100)
      : 0;
...
Geography: ${
    customerSegments.geography.markets.join(", ")
...
Revenue Streams:
${
    economics.revenueStreams.map((s) =>
      `  • ${s.type}: ${s.description} (${s.percentage}%)`
    ).join("\n")
  }
...
Funding Sources:
${economics.fundingSources.map((s) => `  • ${s.type}: €${s.amount}`).join("\n")}
Amount Seeking: €${economics.amountSeeking}
Use of Funds:
${economics.useOfFunds.map((u) => `  • ${u.item}: €${u.amount}`).join("\n")}
...
🔟 KEY PARTNERS:
${
    resources.partners.map((p) => `  • ${p.type}: ${p.name} - ${p.value}`).join(
      "\n",
    )
  }
...

{
  ...
  "growthRoadmap": {
    ...
    "month12": { /* same structure */ },
    "month24": { /* same structure */ }
  },
...
- `valueProposition.solution` - Separate solution description
- `valueProposition.advantages[]` - Key advantages list
- `channels.cacDescription` - CAC breakdown explanation
- `economics.revenueStreams[]` - Structured revenue streams
- `economics.revenuePricing` - Pricing strategy
- `economics.arpu` - Average Revenue Per User
- `economics.customerLifetime` - Customer lifetime in months
- `economics.contributionMargin` - Contribution margin
- `resources.needed` - Needed resources
- `resources.dependencies` - Dependencies
- `resources.activities.*` - 5 key activities
- `resources.partners[]` - Key partners
- `competition.whyChooseYou` - Competitive advantage
- `competition.defensibility` - Defensibility
- `growth.traction` - Current traction
- `growth.newMarkets` - New markets plan
- `growth.paybackPeriod` - Payback period forecast
- `growth.targets12/24/36Months` - Specific targets

### ❌ Removed Fields:

- `costBreakEven` - Calculated, not stored
- `costBurnRate` - Calculated, not stored
- `teamAdvisors` - Not in form
- `resourcesHuman/Financial/Network` - Consolidated

### 🔄 Calculated Metrics (shown in prompt):

- LTV = ARPU × Lifetime × (Margin/100)
- LTV/CAC Ratio with color coding
- Payback Period with benchmarks
- Monthly Burn Rate

## Usage Example

```typescript
import { generateAnalysisPrompt } from "@/prompts/FORM_PROMPT";
import { mapFormToDatabase } from "@/utils/mapFormToDatabase";

// Get form data
const formData = useProjectStore().projectData;
